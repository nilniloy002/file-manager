...
class CiTab
{
    ...
    public function getNav()
    {
        /* return "<li class='{$this->activeClass()} {$this->errorClass()}'>
         ...
         return "<a class='nav-link {$this->activeClass()} {$this->errorClass()}' id='{$this->name}-tab' data-toggle='pill' href='#{$this->name}' role='tab' aria-controls='{$this->name}' aria-selected='{ ($this->active) ? true : false }'>{$this->label}</a>";
    }
    ...
    {
        $html = "<div class='tab-pane fade show {$this->activeClass()}' id='{$this->name}' role='tabpanel' aria-labelledby='{$this->name}-tab'>";

        $html .= "<div class='card'>";
        $html .= "<div class='card-header'><div class='card-title'>{$this->label}</div></div>";
        ...
        $html .= '</div>';
        $html .= '<div class="card-action text-right">'.view('admin::include.form.footer')->render().'</div>';
        return $html .= '</div></div>';
    }
}
