...
class ActivityController extends Controller
{
    ...
    protected function getActivityUser($entity,$for='')
    {
        ...
        if(!empty($properties))
        {
            ...
            if(isset($properties->subject) && ($entity->log_name=='role' || $entity->log_name=='fileExtension' || $entity->log_name=='folder'))
            {
    ...
    protected function getActivityDescription($entity)
    {
        ...
        //fileFolder
        if($entity->log_name=='folder' && $entity->description=='created'){
            $name=$this->getActivityUser($entity,$for='subject');
            return 'Created a new folder named - '.$name;
        }
        if($entity->log_name=='folder' && $entity->description=='updated'){
            $name=$this->getActivityUser($entity,$for='subject');
            return 'Updated folder named - '.$name;
        }
        if($entity->log_name=='folder' && $entity->description=='deleted'){
            $name=$this->getActivityUser($entity,$for='subject');
        ...
        if($entity->log_name=='file' && $entity->description=='updated'){
            $name=$this->getActivityUser($entity,$for='subject');
            return 'Update details of file named - '.$name;
        }
